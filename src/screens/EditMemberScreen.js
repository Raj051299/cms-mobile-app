...
import {
  View, Text, TextInput, TouchableOpacity, ScrollView, Image, StyleSheet, Alert, Platform,SafeAreaView
} from 'react-native';
...
const EditMemberScreen = () => {
  ...
  const [member_relationship, setMemberRelatioship] = useState(member?.member_relationship || '');
  const [ice_telephone, setIceTelephone] = useState(member?.ice_telephone || '');
  const [interest_group_1, setInterestGroup1] = useState(member?.interest_group_1 || '');
  const [interest_group_2, setInterestGroup2] = useState(member?.interest_group_2 || '');
  const [interest_group_3, setInterestGroup3] = useState(member?.interest_group_3 || '');

  const pickImage = async () => {
    const permissionResult = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!permissionResult.granted) {
      Alert.alert('Permission Denied', 'You need to allow permission to access media.');
      return;
    }
  ...
  const renderLabeledInput = (label, value, setValue, placeholder, keyboardType = 'default') => (
    <>
...
<ScrollView contentContainerStyle={styles.container}>
      ...
      {renderLabeledInput("Full Name", member_name, setMemberName, "Full Name")}
      <TouchableOpacity style={styles.imageButton} onPress={pickImage}>
        <Text>{coverImage ? "Change Image" : "Pick Image"}</Text>
      </TouchableOpacity>
      {coverImage ? <Image source={{ uri: coverImage }} style={styles.imagePreview} /> : null}
      ...
      {renderLabeledInput("Financial", financial, setFinancial, "Financial Status")}
      ...
      {renderLabeledInput("Telephone", telephone, setTelephone, "Telephone", "phone-pad")}
      {renderLabeledInput("Mobile", mobile, setMobile, "Mobile", "phone-pad")}
      {renderLabeledInput("Email", email, setEmail, "Email", "email-address")}
      {renderLabeledInput("Emergency Contact Name", ice_contact, setIceContact, "ICE Name")}
      {renderLabeledInput("Relationship", member_relationship, setMemberRelatioship, "Relationship")}
      {renderLabeledInput("Emergency Contact Phone", ice_telephone, setIceTelephone, "ICE Phone", "phone-pad")}
      {renderLabeledInput("Interest Group 1", interest_group_1, setInterestGroup1, "Interest Group 1")}
      {renderLabeledInput("Interest Group 2", interest_group_2, setInterestGroup2, "Interest Group 2")}
      {renderLabeledInput("Interest Group 3", interest_group_3, setInterestGroup3, "Interest Group 3")}
